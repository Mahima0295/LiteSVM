...
public class TestRBFs {
    ...
    @Test
    public void testXORSamplesWithRBF() throws Exception {
        ...
        final List<BinaryTrainingSample> samples = Lists.newArrayList(
                ...
                new BinaryTrainingSample(true, 1, 0),
                new BinaryTrainingSample(false, 1, 1)
        );

        final BinaryClassifier classifier = LiteSVM.trainBinaryClassifier(samples, Kernels.RBF(0.5));
        assertFalse(classifier.predict(0, 0));
        assertTrue(classifier.predict(0, 1));
        assertTrue(classifier.predict(1, 0));
        assertFalse(classifier.predict(1, 1));
    }


    @Test
    public void testHorizontalLineSamplesWithRBF() throws Exception {
    ...
    @Test
    public void testVerticalLineSamplesWithRBF() throws Exception {
